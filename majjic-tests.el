...
(ert-deftest majjic-test-log-buffer-name-uniquifies-same-basename-repos ()
  "Log buffers for same-named repositories should get path suffixes."
  (let* ((base-one (make-temp-file "majjic-buffer-one-" t))
         (base-two (make-temp-file "majjic-buffer-two-" t))
         (repo-one (expand-file-name "repo" base-one))
         (repo-two (expand-file-name "repo" base-two))
         buffers)
    (make-directory repo-one)
    (make-directory repo-two)
    (unwind-protect
        (let ((buffer-one (majjic--generate-log-buffer repo-one))
              buffer-two)
          (push buffer-one buffers)
          (should (equal (buffer-name buffer-one) "majjic: repo"))
          (with-current-buffer buffer-one
            (majjic-log-mode)
            (setq majjic--repo-root repo-one))
          (setq buffer-two (majjic--generate-log-buffer repo-two))
          (push buffer-two buffers)
          (with-current-buffer buffer-two
            (majjic-log-mode)
            (setq majjic--repo-root repo-two))
          (should-not (equal (buffer-name buffer-one)
                             (buffer-name buffer-two)))
          (should (string-match-p "\\`majjic: repo<.+>\\'"
                                  (buffer-name buffer-one)))
          (should (string-match-p "\\`majjic: repo<.+>\\'"
                                  (buffer-name buffer-two))))
      (mapc (lambda (buffer)
...
(ert-deftest majjic-test-log-buffer-reuses-renamed-buffer-for-same-repo ()
  "Reopening a repo should reuse its buffer after uniquify renames it."
  (let* ((base-one (make-temp-file "majjic-buffer-reuse-one-" t))
         (base-two (make-temp-file "majjic-buffer-reuse-two-" t))
         (repo-one (expand-file-name "repo" base-one))
         (repo-two (expand-file-name "repo" base-two))
         buffers)
    (make-directory repo-one)
    ...
    (unwind-protect
        (let ((buffer-one (majjic--generate-log-buffer repo-one))
          ...
          (with-current-buffer buffer-two
            (majjic-log-mode)
            (setq majjic--repo-root repo-two))
          (should (eq (majjic--get-log-buffer repo-one) buffer-one))
          (should (eq (majjic--get-log-buffer repo-two) buffer-two)))
      (mapc (lambda (buffer)
              (when (buffer-live-p buffer)
                (kill-buffer buffer)))
            buffers))))
...
(ert-deftest majjic-test-divergent-change-marks-use-offset-revsets ()
  "Divergent changes should use jj's `/N' revset suffix when marking rows."
  (skip-unless (executable-find "jj"))
  (let* ((repo (make-temp-file "majjic-divergent-" t))
         (default-directory repo))
    (majjic-test--jj repo "git" "init" ".")
    (majjic-test--jj repo "describe" "-m" "base")
    (majjic-test--jj repo "new" "-m" "topic")
    (let ((op (majjic-test--jj repo "op" "log" "--no-graph" "--color" "never"
                                "--template" "id.short() ++ \"\\n\"" "--limit" "1")))
      (majjic-test--jj repo "describe" "-r" "@" "-m" "one")
      (majjic-test--jj repo "--at-op" op "describe" "-r" "@" "-m" "two")
      (let* ((majjic--repo-root repo)
             (records (seq-filter
                       (lambda (record)
                         (and (eq (majjic-revision-kind record) 'revision)
                              (string-match-p "\\(?:one\\|two\\)"
                                              (or (majjic-revision-summary record) ""))))
                       (majjic--read-log-records)))
             (change-ids (mapcar #'majjic-revision-change-id records)))
        (should (= (length records) 2))
        (should (= (length (delete-dups (copy-sequence change-ids))) 2))
        (should (seq-some (lambda (change-id)
                            (string-match-p "/0\\'" change-id))
                          change-ids))
        (should (seq-some (lambda (change-id)
                            (string-match-p "/1\\'" change-id))
                          change-ids))
        (dolist (record records)
          (should (equal (majjic--change-id-for-commit-id
                          (majjic-revision-commit-id record))
                         (majjic-revision-change-id record)))
          (should (equal (majjic--commit-id-for-change-id
                          (majjic-revision-change-id record))
                         (majjic-revision-commit-id record))))
        (with-temp-buffer
          (majjic-log-mode)
          (let ((majjic--repo-root repo))
            ...
            (let ((first (car records))
                  (second (cadr records)))
              (majjic--goto-commit (majjic-revision-commit-id first))
              (majjic-toggle-mark)
              (should (equal (majjic--marked-visible-change-ids)
                             (list (majjic-revision-change-id first))))
              (should (equal (majjic--marked-visible-commit-ids)
                             (list (majjic-revision-commit-id first))))
              (majjic--goto-commit (majjic-revision-commit-id second))
              (majjic-toggle-mark)
              (should (equal (sort (copy-sequence
                                    (majjic--marked-visible-change-ids))
                                   #'string<)
                             (sort change-ids #'string<))))))))))
...
(ert-deftest majjic-test-prefix-arg-prompts-for-directory ()
  "`C-u M-x majjic' should read the repository directory explicitly."
    ...
    (unwind-protect
        (let ((default-directory start-dir)
          ...
          (cl-letf (((symbol-function 'majjic--locate-root)
                     (lambda (dir)
                       (when (equal dir picked-dir)
                         picked-dir)))
                    ((symbol-function 'read-directory-name)
                     (lambda (_prompt dir &rest _args)
                       (setq prompted t)
                       (setq prompt-default dir)
                       picked-dir))
            ...
            (call-interactively #'majjic))
          (should prompted)
          (should (equal prompt-default start-dir))
